...
function pprint.pformat(obj, option, printer)
    ...
    local function make_fixed_formatter(t, has_cache)
        if has_cache then
            ...
            return function(v) return '[[' .. t .. ']]' end
        end
    end
    ...
    local function table_formatter(t)
        ...
        if option.object_cache then
            ...
            if cache_state == false then
                ...
                return
                    string_formatter(string.format('%s %d', ttype, tix), true)
        ...
        local function print_kv(k, v, t)
            ...
            if formatter[type(v)] == nop_formatter or formatter[type(k)] ==
                nop_formatter or
        ...
        if option.sort_keys then
            ...
            for k, _ in pairs(t) do
                if is_hash_key(k) then table.insert(keys, k) end
            end
            table.sort(keys, cmp)
            for _, k in ipairs(keys) do print_kv(k, t[k], t) end
        else
            for k, v in pairs(t) do
                if is_hash_key(k) then print_kv(k, v, t) end
            end
        end
