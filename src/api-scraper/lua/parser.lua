...
local function process_chapter_context(doctext, idx)
    local count, chapters, spok_lang =
        ultraschall.Docs_GetUSDocBloc_ChapterContext(doctext, idx)
...
local function process_functioncall(doctext, idx)
    local functioncall, prog_lang = ultraschall.Docs_GetUSDocBloc_Functioncall(
                                        doctext, idx)
...
local function process_params(doctext, idx)
    local param_count, params, markup_type, markup_version, prog_lang,
          spok_lang, indent = ultraschall.Docs_GetUSDocBloc_Params(doctext,
                                                                   true, idx)
...
local function process_return_values(doctext, idx)
    local param_count, params, markup_type, markup_version, prog_lang,
          spok_lang, indent = ultraschall.Docs_GetUSDocBloc_Retvals(doctext,
                                                                    true, idx)
    return {
        param_count = param_count,
        entries = parse_params_or_retvals(params),
...
local function process_requires(doctext)
    local count, requires, requires_alt =
        ultraschall.Docs_GetUSDocBloc_Requires(doctext)
...
local function parse_lua_method_signature(functioncall_string)
    ...
    local _, _, method_name = string.find(functioncall_string,
                                          method_identifier_regex)
    ...
    if not string.find(functioncall_string, "=") then
        local parameters = string.gmatch(functioncall_string,
                                         type_identifier_regex)
    ...
    local return_val_string, methodcall =
        table.unpack(split(functioncall_string, "="))
    ...
    if #return_vals == 1 then
        ...
        signature.return_values[#signature.return_values + 1] =
            {type = type_name, identifier = identifier}
        return signature
    end

    for type_name, identifier in string.gmatch(return_val_string,
                                               type_identifier_regex) do
        signature.return_values[#signature.return_values + 1] =
            {type = type_name, identifier = identifier}
    end
...
local function process_USDocML_text(filepath)
    ...
    for idx, entry in ipairs(docblocks) do
        ...
        for function_idx = 1, 4 do
            ...
            if func ~= nil and func.prog_lang ~= nil then
                ...
                if func.prog_lang == "lua" then
                    signatures.lua = parse_lua_method_signature(
                                         func.functioncall)
                end
            end
        end
        ...
        local table = {
            ...
            description = description,
            chapers = chapter_context.chapters,
...
local function main()
    ...
    local sections_to_filter = {
        "1 Introduction to ReaScript", "2.1 CPP Api-Description",
        "2.2 EEL Api-Description", "2.3 Python Api-Description",
        "2.4 Lua Api-Description", "3 Datatypes used in this document"
    }

    local function not_an_intro_section(entry)
        for _, section in pairs(sections_to_filter) do
            if entry.title == section then return false end
        end
        return true
    end

    local reascript_core_api = table.filter(unfiltered_api, function(it)
